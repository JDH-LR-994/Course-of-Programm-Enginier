...
while (1) {
    ...
    if (fork() == 0) { // Дочерний процесс для обработки клиента
        close(sock); // Закрываем слушающий сокет в дочернем процессе
        printf("guess_word start");    
        guess_word(fd, fd);
        close(fd);
        exit(0); // Завершаем дочерний процесс
    }
    close(fd); // Закрываем клиентский сокет в родительском процессе
}
}
